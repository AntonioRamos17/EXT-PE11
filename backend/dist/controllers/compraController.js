...
export const getCompras = async (req, res) => {
    try {
        ...
        let condition = comprasEncontrados.length === 0;
...
export const getComprasID = async (req, res) => {
    try {
        ...
        let condition = compraEncontrado === null;
...
export const postCompras = async (req, res) => {
    try {
        ...
        const productosEstructurados = await Promise.all(productos_.map(async (producto) => {
 ...
 */
export const deleteComprasID = async (req, res) => {
    try {
        const compraEliminado = await compraModel.findOneAndDelete({ id_: req.params.id });
        let condition = compraEliminado === null;
        if (compraEliminado) {
            // Restaurar el stock de los productos
            await Promise.all(compraEliminado.productos_.map(async (producto) => {
