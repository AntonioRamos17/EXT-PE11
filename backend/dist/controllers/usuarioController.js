 ...
 */
export const getUsuarioClaves = async (req, res) => {
    try {
...
export const getUsuarios = async (req, res) => {
    ...
    try {
        ...
        let condition = usuariosEncontrados.length === 0;
...
export const getUsuariosID = async (req, res) => {
    try {
        ...
        let condition = usuarioEncontrado === null;
...
export const postUsuariosLogin = async (req, res) => {
    ...
    const token = jwt.sign({ rol: user.rol_, nombre_usuario: user.claves_[0], contrasena: user.claves_[1] }, process.env.JWT_SECRET, {
        expiresIn: '1h',
    });
 ...
 */
export const deleteUsuariosID = async (req, res) => {
    try {
        const usuarioEliminado = await usuarioModel.findOneAndDelete({ id_: req.params.id });
        let condition = usuarioEliminado === null;
