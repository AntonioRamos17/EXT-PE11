...
export const getVentas = async (req, res) => {
    try {
        ...
        let condition = ventasEncontrados.length === 0;
...
export const getVentasID = async (req, res) => {
    try {
        ...
        let condition = ventaEncontrado === null;
 ...
 */
export const deleteVentasID = async (req, res) => {
    try {
        const ventaEliminado = await ventaModel.findOneAndDelete({
            id_: req.params.id
        });
        let condition = ventaEliminado === null;
        if (ventaEliminado) {
            // Restaurar el stock de los productos
            await Promise.all(ventaEliminado.productos_.map(async (producto) => {
